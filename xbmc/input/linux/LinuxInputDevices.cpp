...
bool CLinuxInputDevice::AbsEvent(const struct input_event& levt, XBMC_Event& devt)
{
  ...
  {
  case ABS_X:
    if(levt.value != CInputManager::Get().GetRawX())
    {
      CInputManager::Get().SetRawX(levt.value);
    }
    if(swap_axes)
  ...
  case ABS_Y:
    if(levt.value != CInputManager::Get().GetRawY())
    {
      CInputManager::Get().SetRawY(levt.value);
    }
    if(swap_axes)
      m_mouseX = (int)(((float)levt.value) * (calib_x_fact)) + calib_x_d;
    else
